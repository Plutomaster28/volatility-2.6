...
class SaveConfig(kdbgscan.KDBGScan): # common.AbstractWindowsCommand):
    ...
    def calculate(self):
        ...
        if os.path.isfile(self.save_location):
            resp = input("Are you sure you want to overwrite {}? [Y/n] ".format(self.save_location))
            if resp.upper() == 'N':
                self.abort = True
        ...
        config_saver = ConfigSaver(self.save_location, self.new_config, self.abort, self.suglist)
        config_saver.save_config()


class ConfigSaver:
    def __init__(self, save_location, new_config, abort, suglist):
